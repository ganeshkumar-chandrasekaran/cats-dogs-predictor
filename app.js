(() => {
  ...
  async function loadModel() {
    try {
      ...
      setStatus("Loading MiniResNet model…");
      session = await ort.InferenceSession.create(MODEL_URL, {
        executionProviders: ["wasm"],
      });
      setStatus("Model ready. Upload a cat or dog image.");
    } catch (err) {
      console.error(err);
      setStatus("Failed to load model. Check that model/miniresnet_cats_dogs.onnx is deployed.");
    }
  }
  ...
  async function predict() {
    ...
    try {
      ...
      labelOut.textContent = `${CLASS_NAMES[predIdx]}  ·  ${(confidence * 100).toFixed(1)}%`;
      ...
      setStatus("Prediction complete. Image cleared from memory.");
    } catch (err) {
      console.error(err);
